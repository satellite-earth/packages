...
const iconsDist = './src/components/icons/components';
const iconsList = await fs.readdir(iconsSrc);

for (const filename of iconsList) {
	const content = await fs.readFile(path.join(iconsSrc, filename), {
		encoding: 'utf-8',
	});
	const componentName = camelcase(path.basename(filename, '.svg'), {
		pascalCase: true,
	});
